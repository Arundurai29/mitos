...
const UploadPage = () => {
  ...
  const [data, setData] = useState<any[]>([]); // State for all data types (questionTypes, subjects, chapters, topics)
  const [filteredData, setFilteredData] = useState<any[]>([]); // Filtered data based on search input
  const [searchQuery, setSearchQuery] = useState("");
  const [filterType, setFilterType] = useState<string>("questionType"); // Default to 'questionType'
  const [editingItem, setEditingItem] = useState<any>(null); // Item being edited
  ...
  const fetchParentData = async (type: string) => {
