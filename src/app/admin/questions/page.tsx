...
// Define the structure for the filters and questions
  ...
  useEffect(() => {
    const fetchFilters = async () => {
      try {
        ...
        setTopics(topicRes.data.map((t: any) => ({ value: t.id, label: t.name })));

        const subjectRes = await axios.get("http://localhost:5000/api/subjects", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setSubjects(subjectRes.data.map((s: any) => ({ value: s.id, label: s.name })));

        const chapterRes = await axios.get("http://localhost:5000/api/chapters", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setChapters(chapterRes.data.map((c: any) => ({ value: c.id, label: c.name })));

        const questionTypeRes = await axios.get("http://localhost:5000/api/question-types", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setQuestionTypes(questionTypeRes.data.map((qt: any) => ({ value: qt.id, label: qt.name })));
      } catch (error) {
